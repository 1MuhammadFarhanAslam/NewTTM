...
class AIModelController:
    def __init__(self):
        ...
        self.service = service_flags
        self.last_run_start_time = dt.datetime.now()
        self.wandb_run = None  # Initialize wandb_run
...
async def setup_and_run(controller, with_api):
    tasks = []
    if with_api and os.path.exists(os.path.join(project_root, 'app')):
        secret_key = os.getenv("AUTH_SECRET_KEY")
        if not secret_key:
            raise ValueError("Auth Secret key not found in environment variable AUTH_SECRET_KEY")
        app = create_app(secret_key)
...
async def main():
    ...
    # Ask the user if they want to start with or without API
    choice = input("Do you want to start the validator with API? (yes/no): ").strip().lower()

    if choice == 'yes':
        with_api = True
        if not os.getenv("AUTH_SECRET_KEY"):
            secret_key_input = input("Please provide your AUTH_SECRET_KEY: ").strip()
            os.environ["AUTH_SECRET_KEY"] = secret_key_input
    else:
        with_api = False

    await setup_and_run(controller, with_api)
